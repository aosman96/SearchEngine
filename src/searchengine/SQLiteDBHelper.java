        ...
        public SQLiteDBHelper(int memory, String path,int isCrawler)
        {
            ...
            try
            {
                ...
                if (isCrawler == 1)
                {
                    ...
                    query = "CREATE TABLE IF NOT EXISTS Documents(ID INTEGER PRIMARY KEY AUTOINCREMENT, URL TEXT, Content TEXT, RANK FLOAT DEFAULT 0, InLinksCount INT DEFAULT 0, OutLinksCount INT DEFAULT 0, Visited TINYINT);";
                    stmt.execute(query);
                ...
                else if (isCrawler == 0)
                {
                    ...
                    stmt.execute(query);
                    query = "CREATE TABLE IF NOT EXISTS WordInfo(ID INTEGER PRIMARY KEY AUTOINCREMENT, Original INTEGER, URL TEXT, TF FLOAT, RANK INT, Positions TEXT, Sentence TEXT);";
                    stmt.execute(query);
...
*/
        ...
        {
            String query = "SELECT ID,URL,Content FROM Documents WHERE Visited = 1 AND Content != \"NULL\" AND ID>? LIMIT 1000";
            ResultSet rs = null;
        ...
        public void CalculateIDF()
        {
            ...
            String query2 = "select ID from Words;";
            String query3 = "select Count(*) AS totalcount2 from WordInfo where Original = ?;";
            String query4 = "UPDATE Words SET IDF = LOG10(CAST(? AS FLOAT)/CAST(? AS FLOAT)) WHERE ID = ?";
            int count2;
            PreparedStatement pstmt;
        ...
        public void UpdatePagesToVisit(Set<webPage> pagesToVisit)
        {
            ...
            String query = "UPDATE Documents SET Visited = ?, Content = ? WHERE URL = ?";
            PreparedStatement pstmt;
                ...
                for (webPage pageToVisit : pagesToVisit)
                    {
                        ...
                        if(count > 0)
                        {
                            ...
                            pstmt.setString(3, pageToVisit.getUrl());
                            pstmt.addBatch();
                        }
                        else if(count == 0)
                        {
        ...
        public void InsertWords(HashMap<String, HashMap<String, Double>> OriginalWordRank, HashMap<String, HashMap<String, HashMap<String, wordInfo>>> IndexInfo)
        {
            ...
            try {
                ...
                //get positions
                query2 = "SELECT ID FROM WordInfo WHERE URL = ?";
